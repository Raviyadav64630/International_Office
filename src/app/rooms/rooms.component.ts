...
export class RoomsComponent implements OnInit {
  ...
  rooms2: Room[]; // data from  DB
  rooms3: Room[]; // data used here
  selectedRoom: Room;

  constructor(private httpService: HttpService,) {
    let myUrl = environment.SOCKET_ENDPOINT + "/rooms";
    // let myUrl = "https://openmensa.org/api/v2/canteens/198/days/2021-11-04/meals";
    this.rooms3 = []; // initialize Room Array
    console.log("Rooms component: Sending request");
    this.httpService.sendGetRoomsRequest(myUrl).subscribe(
      (responseBody) => {
        this.rooms2 = responseBody; // get rooms array
        console.log(responseBody);
        console.log(typeof responseBody); // Arrays are Objects in JS
        console.log(responseBody[0].name); // must be a Room object
        
        this.rooms2.forEach(robject => {
          this.rooms3.push({ id: robject.id, name: robject.name, description: robject.name });
        });
